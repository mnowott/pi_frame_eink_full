...
* **Domain:** `nowottnm.de`
* **Public app URL:** `https://app.nowottnm.de`
* **DNS:** Route 53 hosted zone for `nowottnm.de`
...
Hosted zone: `nowottnm.de`

Record for the app:

* **Name:** `app.nowottnm.de`
* **Type:** `A`
* **Alias:** Yes
* **Target:** `dualstack.<your-alb-name>.<region>.elb.amazonaws.com.` (ALB DNS name)

Make sure your domain registrar uses the Route 53 name servers for `nowottnm.de`.
...
* Attach this cert to the ALB HTTPS listener:
  ...
  * Listener 443 → **Standard-SSL/TLS-Serverzertifikat**: `*.nowottnm.de`
...
This forces all `http://app.nowottnm.de` → `https://app.nowottnm.de`.
...
* **Protocol:** HTTPS
* **Port:** 443
* **SSL certificate:** `*.nowottnm.de` (ACM)
...
* Under **Authentication**:
  ...
  * Add redirect URI:
    ...
    ```text
    https://app.nowottnm.de/oauth2/idpresponse
    ```
...
* **Users and groups:**
  ...
  * Assign only the users/groups allowed to access `app.nowottnm.de`
...
* **Session cookie name:**
  ...
  * Something like: `AWSELBAuthAppNowottnm`
...
```bash
openssl s_client -connect app.nowottnm.de:443 -servername app.nowottnm.de </dev/null 2>/dev/null | openssl x509 -noout -subject -issuer
```

* Subject should show `CN=*.nowottnm.de`
* Issuer should be Amazon

### 8.3 HTTP Flow

* Open `http://app.nowottnm.de`:

  * Should redirect to `https://app.nowottnm.de`
* Open `https://app.nowottnm.de` (Incognito):
